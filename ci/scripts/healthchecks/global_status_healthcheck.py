...
#UserCheck
def userCheck():
    command = "/opt/quest/bin/vastool user checkaccess ossadmin|awk \'/ossadmin/ {print}\'|cut -d \" \" -f 1"
    stdoutdata, stderrdata, proc = exProces(command)
    if stdoutdata !='ALLOWED\n':
        with open(health_check_report, "a") as outputFile:
            outputFile.write("GLOBAL_USER: - Connection_Unsuccessful\n")
    else:
        with open(health_check_report, "a") as outputFile:
            outputFile.write("GLOBAL_USER: - Connection_Successful\n")
...
def main():
    ...
    if functionName == "GlobalStatusCheck":
        userCheck()
