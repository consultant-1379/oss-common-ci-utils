...
# Variables
# GE
grid_engine_instances = ["26249","25612","25159","24896","26247","25746","00297","06842", "22354", "22394", "22395", "22399", "22400", "24758", "22342", "26934", "27025","19206","19511"]
ge_login_command = "qrsh -q ossmsci.q -l hostname=seliiusGE.seli.gic.ericsson.se"
file_system_command = " df -h " # Disk space usage command
docker_folder = "/local1"
...
# GE Healthcheck
def gridEngAccessCheck():
    for instance in grid_engine_instances:
        instanceName = "seliius" + instance
        command = ge_login_command.replace("GE",instance) + file_system_command + docker_folder
        stdoutdata, stderrdata, proc = exProces(command)

        if stderrdata:
            with open(health_check_report, "a") as outputFile:
                outputFile.write("GE: " + instanceName + " - Connection_Unsuccessful\n")

        if stdoutdata:
            with open(health_check_report, "a") as outputFile:
                outputFile.write("GE: " + instanceName + " - Connection_Successful\n")
