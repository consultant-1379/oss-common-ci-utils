...
pipeline {
    ...
    environment {
        log_shipper_sidecar_tag = "${env.VERSION}"
    }
    ...
    stages {
        ...
        stage('Generate CR for Log Shipper Sidecar') {
            steps {
                    ...
                    script {
                        ...
                        if (yamlFiles) {
                            yamlFiles.each { yamlFile ->
                                ...
                                // Check if 'eric-log-shipper-sidecar' is present in the file
                                if (yamlContent.contains('eric-log-shipper-sidecar')) {
                                    def lines = yamlContent.readLines()
                                    // Flag to identify if 'tag' should be updated
                                    def updateTag = false
                                    // Iterate over each line
                                    lines.eachWithIndex { line, index ->
                                        // Check if the line contains 'eric-log-shipper-sidecar'
                                        if (line.contains('eric-log-shipper-sidecar')) {
                                        ...
                                        if (updateTag && line.contains('tag:')) {
                                            ...
                                            def logShipperSidecarTag = env.log_shipper_sidecar_tag.replaceAll("\\n|\\r", "")
                                            lines[index] = line.replaceAll(/tag: .*/, "tag: \"${logShipperSidecarTag}\"")
                                    ...
                                    sh '''
                                        ...
                                        git add .
                                        git commit -m "NO-JIRA: Automatic 2pp version uplift for eric-log-shipper-sidecar to ''' + log_shipper_sidecar_tag + '''"
                                        git push origin HEAD:refs/for/master
                                    '''
                                } else {
                                    echo "Variable 'eric-log-shipper-sidecar' not found in YAML file '${yamlFile.path}'. Skipping Gerrit patch creation."
                                }
                            }
                        } else {
    ...
    post {
        success {
            mail to: "${env.EMAIL}",// sending CR link and notification to teams
                 ...
                 subject: "2pp version uplift for eric-log-shipper-sidecar to ${env.log_shipper_sidecar_tag} for the '${artifactID}'",
                 body: "Hi Team,<br><br>" +
                       "Code review has been created for automated Log Shipper Sidecar version uplift to the latest version :  ${env.log_shipper_sidecar_tag}." +
        ...
        failure {
            mail to: "b32734be.ericsson.onmicrosoft.com@emea.teams.ms", //sending failure notification to HB
                 from: "jenkins-ossadmin-no-reply@ericsson.com",
                 subject: "2pp version uplift for eric-log-shipper-sidecar failed for the '${artifactID}'",
                 body: "Hi Team,<br><br>" +
                       "The automatic 2pp version uplift for 'eric-log-shipper-sidecar' has failed for the repository" +
                       "<br>'Log Shipper Sidecar' image not used in eric-product-info.yaml for the repository ${env.REPO}." +
                       "<br>This mail has been sent automatically by <a href='${env.BUILD_URL}'>${env.JOB_NAME}</a>" +
                       "<br>BR, The Hummingbirds" +
                       "<br>PDLAEONICC@pdl.internal.ericsson.com<br>",
            mimeType: 'text/html'
