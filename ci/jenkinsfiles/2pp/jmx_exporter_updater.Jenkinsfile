...
pipeline {
    ...
    stages {
        ...
        stage('Generate CR for JMX Exporter') {
            steps {
                    ...
                    script {
                        ...
                        if (yamlFiles) {
                            yamlFiles.each { yamlFile ->
                                ...
                                // Check if eric-data-message-bus-kf-jmx-exporter is present in file
                                if (yamlContent.contains('eric-data-message-bus-kf-jmx-exporter')) {
                                    ...
                                    lines.eachWithIndex { line, index ->
                                        ...
                                        if (line.contains('eric-data-message-bus-kf-jmx-exporter')) {
                                        ...
                                        if (updateTag && line.contains('tag:')) {
                                            ...
                                            def jmxTag = env.jmx_exporter_tag.replaceAll("\\n|\\r", "")
                                            lines[index] = line.replaceAll(/tag: .*/, "tag: \"${jmxTag}\"")
                                    ...
                                    sh '''
                                        ...
                                        git add .
                                        git commit -m "NO-JIRA: Automatic 2pp version uplift for jmx-exporter to ''' + jmx_exporter_tag + '''"
                                        git push origin HEAD:refs/for/master
                                    '''
                                } else {
                                    echo "Variable 'eric-data-message-bus-kf-jmx-exporter' not found in YAML file '${yamlFile.path}'. Skipping Gerrit patch creation."
                                }
                            }
                        } else {
    ...
    post {
        success {
            ...
            subject: "2pp version uplift for JMX exporter to ${env.jmx_exporter_tag} for the '${artifactID}'",
            body: "Hi Team,<br><br>" +
                "Code review has been created for automated JMX exporter version uplift to the latest version :  ${env.jmx_exporter_tag}." +
        ...
        failure {
            ...
            from: "jenkins-ossadmin-no-reply@ericsson.com",
            subject: "2pp version uplift failed for jmx exporter for the '${artifactID}'",
            body: "Hi Team,<br><br>" +
                "The automatic 2pp version uplift for 'Jmx-Exporter' has failed for the repository" +
                "<br>'Jmx-Exporter' image not used in eric-product-info.yaml for the repository ${env.REPO}." +
                "<br>This mail has been sent automatically by <a href='${env.BUILD_URL}'>${env.JOB_NAME}</a>" +
                "<br>BR, The Hummingbirds" +
                "<br>PDLAEONICC@pdl.internal.ericsson.com<br>",
            mimeType: 'text/html'
