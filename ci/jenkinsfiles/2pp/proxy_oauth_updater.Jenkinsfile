...
pipeline {
    ...
    stages {
        ...
        stage('Generate CR for Proxy Oauth') {
            steps {
                    ...
                    script {
                        ...
                        if (yamlFiles) {
                            yamlFiles.each { yamlFile ->
                                ...
                                // Splitting YAML content by lines
                                if (yamlContent.contains('eric-sec-authorization-proxy-oauth2')) {
                                    def lines = yamlContent.readLines()
                                    // Flag to identify if 'tag' should be updated
                                    def updateTag = false
                                    // Iterate over each line
                                    lines.eachWithIndex { line, index ->
                                        // Check if the line contains 'eric-sec-authorization-proxy-oauth2'
                                        if (line.contains('eric-sec-authorization-proxy-oauth2')) {
                                            // Set the flag to update 'tag' after finding eric-sec-authorization-proxy-oauth2 variable'
                                            updateTag = true
                                        }
                                        ...
                                        if (updateTag && line.contains('tag:')) {
                                            ...
                                            def sapTag = env.proxy_oauth_tag.replaceAll("\\n|\\r", "")
                                    ...
                                    sh '''
                                        ...
                                        git add .
                                        git commit -m "NO-JIRA: Automatic 2pp version uplift for proxy-auth to ''' + proxy_oauth_tag + '''"
                                        git push origin HEAD:refs/for/master
                                    '''
                                } else {
                                    echo "Variable 'eric-sec-authorization-proxy-oauth2' not found in YAML file '${yamlFile.path}'. Skipping Gerrit patch creation."
                                }
                            }
                        } else {
    ...
    post {
        success {
            mail to: "${env.EMAIL}", //CR creation notification to teams
                 from: "jenkins-ossadmin-no-reply@ericsson.com",
                 subject: "2pp uplift for proxy-auth image to ${env.proxy_oauth_tag} for the '${artifactID}'",
                 body: "Hi Team,<br><br>" +
                       "Code review has been created for automated 2pp version uplift of proxy-oauth to the latest version :  ${env.proxy_oauth_tag}." +
        ...
        failure {
            mail to: "b32734be.ericsson.onmicrosoft.com@emea.teams.ms", //sending failure notification to HB
                from: "jenkins-ossadmin-no-reply@ericsson.com",
                subject: "2pp version uplift failed for proxy-auth image for the '${artifactID}'",
                body: "Hi Team,<br><br>" +
                       "The automatic 2pp version uplift for 'proxy-oauth' has failed for the repository" +
                       "<br>'proxy-oauth' image not used in eric-product-info.yaml for the repository ${env.REPO}." +
                       "<br>This mail has been sent automatically by <a href='${env.BUILD_URL}'>${env.JOB_NAME}</a>" +
                       "<br>BR, The Hummingbirds" +
                       "<br>PDLAEONICC@pdl.internal.ericsson.com<br>",
            mimeType: 'text/html'
