                    ...
                    script {
                       withCredentials([usernamePassword(credentialsId: 'GERRIT_PASSWORD', usernameVariable: 'GERRIT_USERNAME', passwordVariable: 'GERRIT_PASSWORD')]) {
                            sh '''
                                ...
                                --env REQUEST_TIMEOUT\
                                ${DOCKER_IMAGE}\
                                bash -c "python -u ${SCRIPT} 'SERO Artifactory' 'https://arm.sero.gic.ericsson.se/artifactory/proj-ADP_GS_Support_FTP-generic-local/collect_ADP_logs/'"
                            '''
                        }
                    }
                }
